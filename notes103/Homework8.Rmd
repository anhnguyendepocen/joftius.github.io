...
This question is about whether or not a certain coin is fair. We model the coin as a Bernoulli random variable $C \sim \text{Ber}(p)$, with $P(C = h) = p$ and $P(C = t) = 1-p$ (heads and tails). We toss the coin $n$ times and record the outcomes as $C_1, C_2, \ldots, C_n$. 
...
If we have two hypothesis tests computed at the 5\% significance level, each one individually has a 5\% probability of type 1 error (false rejection). What is the probability of at least one false rejection? The answer should be a number, but show your work.
